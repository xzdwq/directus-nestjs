...
export default defineComponent({
	...
	setup(props) {
		...
		const imageThumbnail = computed(() => {
			if (!props.value) return null;
			if (props.value.type.includes('svg')) return addTokenToURL(`${getAssetsPath()}${props.value.id}`);
			if (props.value.type.includes('image') === false) return null;
			return addTokenToURL(`${getAssetsPath()}${props.value.id}?key=system-small-cover`);
		});
