...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const downloadUrl = computed(() => {
			...
			return addTokenToURL(`${getAssetsPath()}${relatedPrimaryKey.value}?download`);
		});
		...
		function getUrl(junctionRow: Record<string, any>, addDownload?: boolean) {
			...
			if (addDownload) {
				return addTokenToURL(`${getAssetsPath()}${key}?download`);
			}

			return addTokenToURL(`${getAssetsPath()}${key}`);
		}
