<template>
	<private-view :title="t('product_request.orders_system')">
		<template #headline>{{ t('product_request.active_orders') }}</template>
		<!-- Header -->
		...
		<template #actions:prepend>
			<span v-if="ordersStore.getTotalActive > 0" class="relative mr-2 whitespace-nowrap text-[#a2b5cd] font-semibold">
		...
		<!-- Body -->
		<div class="px-9">
			<registry-body :is-reload="isReload" type="active" />
		</div>
		...
		<template #sidebar>
			...
			<actions-form v-if="ordersStore.getSelectionWritable.length > 0" type="active" />
		</template>
	</private-view>
</template>
