...
import { getAssetsPath } from '@/utils/get-root-path';
...
export default defineComponent({
	...
	setup(props) {
		...
		const fileSrc = computed(() => {
			if (item.value && item.value.modified_on) {
				return addTokenToURL(
					`${getAssetsPath()}${props.primaryKey}?cache-buster=${item.value.modified_on}&key=system-large-contain`
				);
			}

			return addTokenToURL(`${getAssetsPath()}${props.primaryKey}?key=system-large-contain`);
		});
		...
		function downloadFile() {
			const filePath = addTokenToURL(`${getAssetsPath()}${props.primaryKey}?download`);
