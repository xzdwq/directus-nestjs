...
const artStyles = computed(() => {
	...
	const url = `${getAssetsPath()}${info.value!.project?.public_background}`;
...
const foregroundURL = computed(() => {
	...
	return `${getAssetsPath()}${info.value.project?.public_foreground}`;
});

const logoURL = computed<string | null>(() => {
	if (!info.value?.project?.project_logo) return null;
	return `${getAssetsPath()}${info.value.project?.project_logo}`;
});
</script>
